    ...
    <div class="contact-main-grid" style="display:grid;grid-template-columns:1.2fr 1fr;gap:80px;align-items:start">
      <div class="contact-form-col reveal">
        <div class="contact-form-header">
         ...
         <form class="cform" action="<?php echo esc_url(get_permalink()); ?>" method="POST">
          <?php wp_nonce_field('acs_contact_action', 'acs_contact_nonce'); ?>
          <input type="hidden" name="acs_contact_submit" value="1">
          <div class="cform-row" style="display:grid;grid-template-columns:1fr 1fr;gap:20px">
            <div class="cform-group"><div class="cform-input-wrap"><input type="text" id="cFullName" name="fullName" required class="cform-input" placeholder=" "><label for="cFullName" class="cform-label">Full Name <span class="cform-req">*</span></label></div></div>
            <div class="cform-group"><div class="cform-input-wrap"><input type="email" id="cEmail" name="email" required class="cform-input" placeholder=" "><label for="cEmail" class="cform-label">Email Address <span class="cform-req">*</span></label></div></div>
          </div>
          <div class="cform-row" style="display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-top:20px">
            <div class="cform-group"><div class="cform-input-wrap"><input type="tel" id="cPhone" name="phone" class="cform-input" placeholder=" "><label for="cPhone" class="cform-label">Phone <span class="cform-opt">(optional)</span></label></div></div>
            <div class="cform-group"><div class="cform-input-wrap"><select id="cService" name="service" class="cform-input cform-select"><option value="" selected disabled hidden></option><option value="assessment">Assessment &amp; Evaluation</option><option value="training">Training &amp; Workshops</option><option value="homebased">Home-Based ABA</option><option value="schoolbased">School-Based ABA</option><option value="telehealth">TeleHealth ABA</option><option value="parents">For Parents</option><option value="other">Other</option></select><label for="cService" class="cform-label">Service of Interest <span class="cform-opt">(optional)</span></label></div></div>
          </div>
          <div class="cform-group cform-group-full" style="margin-top:20px">
            <div class="cform-input-wrap"><textarea id="cMessage" name="message" rows="5" required class="cform-input cform-textarea" placeholder=" "></textarea><label for="cMessage" class="cform-label">Message <span class="cform-req">*</span></label></div>
          </div>
          <?php if (isset($_GET['acs_sent']) && $_GET['acs_sent'] === '1') : ?>
            <div class="cform-alert cform-alert-success" style="margin-top:20px;padding:16px 20px;border-radius:10px;background:#e8f5e9;color:#2e7d32;font-family:var(--sans);font-size:14px;font-weight:500;display:flex;align-items:center;gap:10px"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width:20px;height:20px;flex-shrink:0"><path d="M22 11.08V12a10 10 0 11-5.93-9.14"/><path d="M22 4L12 14.01l-3-3"/></svg>Thank you! Your message has been sent successfully. We'll be in touch within 24 hours.</div>
          <?php elseif (isset($_GET['acs_sent']) && $_GET['acs_sent'] === '0') : ?>
            <div class="cform-alert cform-alert-error" style="margin-top:20px;padding:16px 20px;border-radius:10px;background:#fbe9e7;color:#c62828;font-family:var(--sans);font-size:14px;font-weight:500;display:flex;align-items:center;gap:10px"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width:20px;height:20px;flex-shrink:0"><circle cx="12" cy="12" r="10"/><path d="M15 9l-6 6M9 9l6 6"/></svg>Sorry, something went wrong. Please try again or contact us directly by phone.</div>
          <?php endif; ?>
